...
- Optionally dim non-matching text to make colored words stand out
...
require("kaleidosearch").setup({
  ...
  backdrop = {
    enabled = false,                     -- Dim non-matching text during word highlighting
    highlight_group = "KaleidosearchBackdrop", -- Linked to Comment by default
  },
  keymaps = {
...
*** Dimming non-matching text

Enable the backdrop to make colored matches stand out against muted surrounding text:
...
The backdrop is buffer-local and applies to word highlighting, including the all-words modes. Clearing highlights, toggling off the last word, or switching to line or token colorization removes it.

~KaleidosearchBackdrop~ links to your colorscheme's ~Comment~ group by default. To choose a specific color, define the group after loading your colorscheme:

#+begin_src lua
vim.api.nvim_set_hl(0, "KaleidosearchBackdrop", { fg = "#666666" })
#+end_src

If you switch colorschemes at runtime, reapply your custom definition in a ~ColorScheme~ autocmd. Alternatively, set ~backdrop.highlight_group~ to an existing highlight group supplied by your theme.
